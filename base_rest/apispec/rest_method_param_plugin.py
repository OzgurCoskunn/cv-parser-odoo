...
class RestMethodParamPlugin(BasePlugin):
    ...
    def _generate_responses(self, routing, method, params):
        ...
        resp = routing.get("responses")
        if resp:
            for res in resp.values():
                if isinstance(res, RestMethodParam):
                    responses.update(
                        res.to_openapi_responses(self._service, self.spec, code=res._code, desc=res._desc)
                    )
