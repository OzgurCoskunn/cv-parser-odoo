...
def _is_dev_mode():
    if config.get("dev_mode") and "base_rest" in config["dev_mode"]:
        return True
    ...
    if rcfile:
        ...
        try:
            cp.read(rcfile)
            if cp.has_option("base_rest", "dev_mode"):
                return cp.getboolean("base_rest", "dev_mode")
        except Exception:
            pass
    return False
...
class RestApiDispatcher(Dispatcher):
    ...
    def pre_dispatch(self, rule, args):
        ...
        if httprequest.mimetype == "application/json":
            data = httprequest.get_data(as_text=True)
            if data:
                try:
