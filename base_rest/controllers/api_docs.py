...
class ApiDocsController(Controller):
    ...
    @route(
        ["/api", "/api/redoc", "/api/<collection>", "/api/redoc/<collection>"],
        methods=["GET"],
        type="http",
        auth="public",
    )
    ...
    @route("/<path:collection>/<string:service_name>.json", auth="public")
    def api(self, collection, service_name):
        with self.service_and_controller_class(collection, service_name) as (service, controller_class):
            doc = service.to_openapi(default_auth=controller_class._default_auth)
            return self.make_json_response(doc)
